...
// Custom homing sequence for the Z/A/X/Y axes, replacing KM_Axis's built-in
// HOMING_ROUTINE_SOURCE_TYPE.AUTO. AUTO's home-task generator has a long
// documented bug in its limit-switch comparison logic (Dynomotion forum,
// "Homing setup and issues") where an axis can keep driving even after its
// limit bit is already reporting the triggered state - confirmed on this
// machine via the live limit-switch indicators on the Manual Control page:
// the bit visibly went active while the axis kept driving into the hard
// stop, for every axis. This program is adapted from Dynomotion's own
// C:\KMotion5.4.5\C Programs\HomeMM_V10.c example (ships with the current
// KMotion install), which implements the same style of stop-on-switch logic
// as plain, inspectable C rather than AUTO's opaque generator.
//
// Sequenced one axis per SequenceId (Z, then A, then X, then Y) so each axis
// fully completes before the next starts - the same order/safety intent as
// the previous AUTO-based RunHomeAll() in Kflop.cs (Z/A home first so the
// nozzle/picker clears before X/Y move).

#if 1 // set to 1 for diagnostic messages - these are already picked up by
      // the app's existing _Controller.MessageReceived handler (Kflop.cs)
      // and shown via Debug.WriteLine, no extra wiring needed.
...
// Jog()/MoveRelAtVel() on the KFLOP work in raw encoder counts (and counts/sec),
// not the "user units" KM_Axis.CPU (=1000 for every axis here, set in
// Kflop.cs's InitDeviceSettings) that HomingParams.HomeFastVel/HomeSlowVel
// used to be expressed in under AUTO. AXIS_CPU=1000 (matching KM_Axis.CPU
// exactly) turned out to be about 1000x too fast in testing - it drove every
// axis past its end stop - so the mapping between KM_Axis.CPU and this raw
// C-level Jog() is NOT the straightforward multiply that was assumed. 40 is a
// safer starting guess (roughly midway, on a log scale, between "far too
// slow" at 1x and "crashes past the end stop" at 1000x), not a calculated
// value - re-tune this single constant and rebuild between each test rather
// than trusting it, and watch the limit-switch indicators + keep a hand on
// E-Stop until it's dialed in.
#define AXIS_CPU 120.0

// Distance (in counts, i.e. already scaled by AXIS_CPU) to back off the
// switch before the slower second-pass approach, and how far off the switch
// the axis ends up sitting when homing completes. This wasn't a tunable
// exposed by AUTO's HomingParams, so it's a fresh conservative starting value
// (3 user units) - if an axis doesn't fully clear the switch before the slow
// re-approach (or backs off further than expected), tune this per-axis by
// giving each CreateHomeTask() call its own offset instead of sharing this
// one constant.
#define BACKOFF_DIST (3.0 * AXIS_CPU)
...
void LoadUserData(void)
{
	...
	// persist.UserData[] survives between program runs on the KFLOP, so a
	// completion bit left set by a previous successful home would otherwise
	// look like an already-finished axis the moment this program starts -
	// clear it before anything is generated/run.
	persist.UserData[HOMING_COMPLETE_FLAGS]=0;

	SequenceId=0;

	// Same axis/switch/speed values Kflop.cs previously configured through
	// KM_Axis.HomingParams (HomeLimitBit/HomeLimitState/HomeNegative/
	// HomeFastVel/HomeSlowVel/RepeatHomeAtSlowerRate/SetToZero). HomeLimitState
	// of 1 matches the confirmed wiring: bit reads low when off the switch,
	// high when triggered.

	//Z first, for safety - matches the previous Jog/StartDoHome order
	// slowspeed/fastspeed below are the old HomeSlowVel/HomeFastVel user-unit
	// values times AXIS_CPU, converting to the raw counts/sec Jog() actually
	// expects - e.g. Z's old HomeSlowVel=70, HomeFastVel=300. These are real
	// expressions (not pre-computed literals) specifically so that changing
	// the single AXIS_CPU constant above actually changes what gets commanded -
	// an earlier version of this file computed these by hand assuming
	// AXIS_CPU=1000 and left them as bare integers, so retuning AXIS_CPU alone
	// silently did nothing to the actual speeds.
	SequenceId = 0;
	AxisChannel = CHAN_AXIS_2;
	CreateHomeTask(
		HOME_TO_LIMIT_THEN_ROLL_OFF,	// home type - back off switch and zero there (SetToZero)
		HOME_DIR_NEGATIVE,	// switch dir
		TRUE,				// repeat home at slower rate
		(int)(70 * AXIS_CPU),	// slowspeed
		(int)(300 * AXIS_CPU),	// fastspeed
		21,					// limitbit
		1,					// limitstate
		0,					// encoderbit
		0,					// origin (unused for ROLL_OFF)
		BACKOFF_DIST);		// sensor offset

	SequenceId = 1;
	AxisChannel = CHAN_AXIS_3;
	CreateHomeTask(
	...
	// X and Y share SequenceId 2 - DoSequence() jogs/polls/backs-off every task
	// in a sequence together rather than one-at-a-time, so these two now home
	// concurrently instead of X-then-Y, after Z and A have already cleared.
	// This is different from the old AUTO-based concurrent homing that caused
	// problems earlier in this project: that bug was in AUTO's own limit-switch
	// comparison logic (confirmed via the Manual Control limit-switch
	// indicators - the bit read correctly while AUTO kept driving anyway), not
	// in the concept of homing two axes at once, which this hand-written
	// DoSequence() loop (adapted from Dynomotion's own HomeMM_V10.c) handles
	// correctly per-task regardless of how many axes share the sequence.
	SequenceId = 2;
	AxisChannel = CHAN_AXIS_0;
	CreateHomeTask(
		HOME_TO_LIMIT_THEN_ROLL_OFF,
		HOME_DIR_NEGATIVE,
		TRUE,
		(int)(80 * AXIS_CPU),	// slowspeed
		(int)(100 * AXIS_CPU),	// fastspeed
		19,					// limitbit
		1,					// limitstate
		0,
		0,
...
int DoSequence(HOMING_PARAMS *Tasks, int Count)
{
	...
	while(GlobalStatus != ON_SENSOR)
	{
		for(j = 0; j < Count; j++)
		{
			// Status != ON_SENSOR guards against re-counting an axis that
			// already tripped its switch on an earlier pass through this
			// while loop - without it, an axis sitting on its now-triggered
			// switch keeps satisfying this condition (and re-incrementing
			// SequenceAxes) on every subsequent poll while OTHER axes in the
			// sequence are still moving, so SequenceAxes reaches Count and
			// this loop exits before every axis has actually stopped.
			// Invisible with Count==1 (one detection == done immediately),
			// but with Count>1 (X+Y sharing a sequence) it exits early while
			// a still-moving axis then gets a reverse/back-off command
			// layered on top of its still-running forward jog.
			if(Tasks[j].Status != ON_SENSOR && ReadBit(Tasks[j].HomeLimitBit) == Tasks[j].HomeLimitState)
			{
	...
	while(GlobalStatus != ON_SENSOR)
	{
		for(j = 0; j < Count; j++)
		{
			// Same re-counting guard as the fast-approach poll loop above -
			// without Status != ON_SENSOR, an axis already resting on its
			// switch keeps decrementing SequenceAxes on every pass while
			// other axes in the sequence are still approaching.
			if(Tasks[j].Status != ON_SENSOR && ReadBit(Tasks[j].HomeLimitBit) == Tasks[j].HomeLimitState)
			{
				Jog(Tasks[j].AxisChannel, 0.0);
				Tasks[j].Status = ON_SENSOR;
		...
		// Diagnostic only (inherited from Dynomotion's HomeMM_V10.c): j is
		// Count here, one past the last valid Tasks[] index the for loop
		// above just used, so Tasks[j].AxisChannel was reading whatever
		// Init() left in the next unused slot (-1) rather than the axis
		// that was actually just checked. Doesn't affect homing itself -
		// this print never gated any motion - just fixing the log noise.
		PRINTF("Axis Stopped axis=%d\n",Tasks[j-1].AxisChannel);
	...
	while(GlobalStatus != MOVING_TO_FINAL)
	{
		for(j = 0; j < Count; j++)
		{
			...
			// Same re-counting guard as the two poll loops above - without
			// Status != MOVING_TO_FINAL, an axis that already cleared its
			// switch keeps satisfying "off the switch" and re-incrementing
			// SequenceAxes on every pass while other axes are still clearing.
			if(Tasks[j].Status != MOVING_TO_FINAL && ReadBit(Tasks[j].HomeLimitBit) != Tasks[j].HomeLimitState)
			{
				Jog(Axis,0.0);
