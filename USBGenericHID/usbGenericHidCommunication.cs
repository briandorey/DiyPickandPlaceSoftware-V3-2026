...
//
//  --- MODIFIED ---
//  Constructor/destructor updated to create and tear down the native
//  notification window (see deviceNotifications.cs) instead of relying on
//  a System.Windows.Forms.Control's handle. No other behavioural changes.
//
//-----------------------------------------------------------------------------
...
namespace usbGenericHidCommunications
    ...
    public partial class usbGenericHidCommunication
        ...
        public usbGenericHidCommunication(int vid, int pid)
            ...
            // Create the native notification window and register it for device notifications
            // (replaces the old "registerForDeviceNotifications(this.Handle)" WinForms-based call)
        ...
        ~usbGenericHidCommunication()
            ...
            // Tear down the native notification window and its message pump thread
